...
3컬럼 UI(파일 트리 / 편집 / 실시간 미리보기)로 작업한다.
...
기본 포트 `http://127.0.0.1:7700`. 자동으로 브라우저가 열린다.

특징:
- 왼쪽: 모든 섹션·페이지 트리. 클릭으로 즉시 로드.
- 가운데: 마크다운 편집 (Ctrl+S 저장, 비저장 상태 표시).
- 오른쪽: 실시간 미리보기 + Mermaid 다이어그램 자동 렌더 + 코드 하이라이트.
- 편집 보조 버튼: TODO 주석·메모·코드 블록·표·Mermaid 템플릿·내부 링크 삽입.
- 스크롤 동기화 옵션.
- "+ 새 페이지" 버튼: 모달로 섹션/제목/슬러그 입력 → 자동 생성 → 즉시 편집.

### CLI 도구 (대안)

GUI 없이 빠르게 페이지를 추가/제거하고 싶을 때.
