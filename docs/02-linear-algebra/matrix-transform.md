...
- **Q1**: 왜 4x4 행렬을 쓰는가? 3x3로 안 되는가? — 3x3는 회전·크기만. 평행이동을 동차좌표 마지막 행/열에 넣으려면 4x4 필요.
- **Q2**: TRS 곱 순서를 SRT로 바꾸면 어떻게 보이는가? — 크기가 회전·이동 전에 적용되므로 비균등 스케일이 이상한 방향으로 늘어남.
- **Q3**: 쿼터니언의 Slerp와 Lerp 차이? — Slerp는 구면(각속도 일정), Lerp는 직선(각속도 비균등). 짧은 보간은 Lerp + 정규화로 대체 가능.
- **Q4**: 부모를 회전했을 때 자식 위치가 따라가는 원리? — `WorldChild = Local * WorldParent`. 부모 트랜스폼이 자식 로컬 공간을 회전·이동시킨다.
- **Q5**: Unreal의 `FRotator`는 왜 존재? — 디자이너용 오일러각 노출. 내부 변환·보간은 `FQuat`으로 변환해서 처리.
