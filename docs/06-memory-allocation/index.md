...
- **Q1**: 일반 `new`가 게임 핫패스에 부적합한 이유? — 락(멀티스레드 안전 위해), 자유 리스트 탐색, 페이지 폴트 가능. ms 단위 변동.
- **Q2**: Pool과 Arena의 사용 시점 차이? — Pool은 같은 크기 객체의 잦은 생성·해제. Arena는 잡다한 임시 메모리를 한꺼번에 해제.
- **Q3**: `FMemStack`의 안전한 사용? — 함수 스코프 안에서만. 반환값으로 포인터 노출 금지(스코프 종료 시 무효화).
- **Q4**: 단편화 측정? — Unreal `stat memory`, `MemReport`. 외부 단편화 비율, 가용 큰 블록 크기.
- **Q5**: 큰 텍스처·메시는 어디서 할당? — 일반 GPU 메모리 + 풀 관리. Streaming pool이 별도(`r.Streaming.PoolSize`).
